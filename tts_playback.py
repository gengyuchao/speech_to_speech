...
def set_vad_playing(playing):
    ...
    if g_audio_manager:
        try:
            ...
            system_logger.error("设置VAD播放状态失败: {}".format(e))
...
def set_vad_sensitivity(sensitivity):
    ...
    if g_audio_manager:
        try:
            ...
            system_logger.error("设置VAD敏感度失败: {}".format(e))
...
def tts_worker():
    ...
    while True:
        ...
        # 格式化序号，保留4位数字前缀
        filename = "{:04d}_{}.wav".format(tts_worker_counter, uuid.uuid4())
...
def play_worker():
    ...
    while True:
        ...
        if stop_playback_flag.is_set():
            system_logger.info("[播放中断] 用户开始说话，跳过当前语音播放")
            stop_playback_flag.clear()
            continue
            
        # 设置播放状态，降低VAD敏感度
        # system_logger.debug("设置播放状态，降低VAD敏感度")
        ...
        try:
            ...
            while sd.get_stream().active:
                if stop_playback_flag.is_set():
                    sd.stop()
                    system_logger.info("[播放中断] 用户开始说话，立即停止播放")
                    stop_playback_flag.clear()
                    break
            ...
            if not stop_playback_flag.is_set():
                # system_logger.debug("[语音完毕]:{}".format(path))
                pass
                
        except Exception as e:
            system_logger.error("[播放错误]: {}".format(e))
            try:
                sd.stop()
            except:
                pass
    ...
    system_logger.info("播放线程循环结束")

def start_tts_threads():
    system_logger.info("启动TTS线程")
