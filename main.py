...
def process_input(text_queue, voice_result_queue):
    ...
    while True:
        ...
        if content:
            system_logger.info(f"收到{'语音' if source == 'voice' else '文字'}输入：{content}")
            try:
                ollama_stream.chat_handle(content, "玉超", tts_playback.submit_text)
            except Exception as e:
                system_logger.error("LLM 处理失败：{}".format(e))
...
def main():
    ...
    system_logger.info("语音识别已启动，你可以开始说话...")
            ...
            if user_prompt.strip().lower().startswith('v'):
                try:
                    ...
                    if 0 <= sensitivity <= 1:
                        ...
                        system_logger.info("VAD敏感度已设置为: {}".format(sensitivity))
                    else:
                        print("敏感度值应在 0-1 之间")
                except ValueError:
