...
class AudioManager:
    ...
    def _listen_loop(self):
        ...
        while self.running:
            ...
            if prob > threshold:
                if not recording:
                    print(f"检测到说话开始... (概率: {prob:.3f}, 阈值: {threshold:.3f})")
                    speech_buffer = []
                    ...
                    if is_similar:
                        print(f"检测到可能是播放的音频回声 (相似度: {similarity_score:.3f})，跳过打断")
                        # 重置状态，不进行打断操作
                        recording = False
                        speech_buffer = []
                        silence_frames = 0
                ...
                if recording:
                    ...
                    if silence_frames >= SILENCE_FRAME_THRESHOLD:
                        ...
                        if is_similar:
                            print(f"语音片段与播放音频相似 (相似度: {similarity_score:.3f})，跳过识别")
                        else:
                            # 保存为 WAV 文件
                            filename = "temp.wav"
                            ...
                            try:
                                text = self.asr.transcribe(
                                    filename,
                                    language="zh",
                                    prompt="这是钟离、温迪和玉超在进行的人工智能方面的技术讨论，其中包括 whisper 和 LLM 模型的内容。"
                                )
                                print("识别结果：", text)
